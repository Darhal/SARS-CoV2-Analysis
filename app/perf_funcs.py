...
if __name__ == '__main__':
    ...
    lev_args = arg_generator(N=17, stride=1, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1])
    funcs_performance_mt_v2([ lev, lev_dp, lev_rec ], args_arr=lev_args, sizes=[0], tick_spacing=1)

    # ###################### NEEDLEMAN ######################
    nw_args = arg_generator(N=5000, stride=10, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], static_args=[[1, -1, -1]], start=1)
    func_performance(needleman, args_arr=nw_args, sizes=[0], tick_spacing=10)

    nw_args = arg_generator(N=6, stride=1, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], static_args=[[0, 0, 0]], start=1)
    funcs_performance([ needleman_all, nw_bio_generic, needleman ], args_arr=nw_args, sizes=[0], tick_spacing=1, 
                    title="Worst Case Performance Comparison (Standard needleman)")

    # nw_args = arg_generator(N=200, stride=5, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], start=1,
