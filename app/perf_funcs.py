...
if __name__ == '__main__':
    ...
    # ###################### LEVENSHTEIN ######################
    lev_args = arg_generator(N=500, stride=100, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1])
    funcs_performance_mt_v2([ lev_rec, lev_dp, lev ], args_arr=lev_args, sizes=[0], tick_spacing=1)


    ###################### NEEDLEMAN ######################
    nw_args = arg_generator(N=1001, stride=10, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], static_args=[[1, -1, -1]], start=1)
    func_performance_mt(needleman, args_arr=nw_args, sizes=[0], tick_spacing=10)

    nw_args = arg_generator(N=200, stride=10, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], static_args=[[1, -1, -1]], start=1)
    funcs_performance_mt_v2([ needleman, needleman_all, nw_bio_generic ], args_arr=nw_args, sizes=[0], tick_spacing=1, 
                    title="Performance comparison (Standard needleman)")

    nw_args = arg_generator(N=200, stride=10, type=STRINGS, samples=NUCLEOTIDES, variant_arg_pos=[0, 1], start=1,
